...
func NewDataPlane(eventSubscriber *EventSubscriber) (*ConsumerDataPlane, error) {
...
func (d *ConsumerDataPlane) startProcessor(ctx context.Context,
	...
	token, found := parseToken(config.TokenKey, &options.SourceDataAddress)
	if !found {
		return nil, errors.New("token not found in endpoint properties")
	}
	channel, found := parseToken(config.ChannelKey, &options.SourceDataAddress)
	if !found {
